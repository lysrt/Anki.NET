...
using Info = System.Tuple<string, string, AnkiNet.Models.FieldList>;
...
internal class Collection
{
    ...
    public Collection(OrderedDictionary infoPerMid, List<AnkiItem> ankiItems, string name)
    {
        ...
        var confFileContent = GeneralHelper.ReadResource("AnkiNet.AnkiData.conf.json");
        _conf = confFileContent.Replace("{MODEL}", mid).Replace("\r\n", "");

        DeckId = DateTimeOffset.Now.ToUnixTimeMilliseconds().ToString();

        var modelsFileContent = GeneralHelper.ReadResource("AnkiNet.AnkiData.models.json").Replace("{MOD}", _mod);
        ...
        var deckFileContent = GeneralHelper.ReadResource("AnkiNet.AnkiData.decks.json");
        _decks = deckFileContent.Replace("{NAME}", name).Replace("{ID_DECK}", DeckId).Replace("{MOD}", _mod).Replace("\r\n", "");

        var dconfFileContent = GeneralHelper.ReadResource("AnkiNet.AnkiData.dconf.json");
